# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

# [1.3.1](https://github.com/Edgelabsolutions/Releasify/compare/1.3.0...1.3.1) (2026-04-07)

### Bug Fixes

* support breaking change syntax and add parser diagnostics ([c289978](https://github.com/Edgelabsolutions/Releasify/commit/c2899782e5c3e19725fbd2757b0a8a584f4e8cc9))

# [1.3.0](https://github.com/Edgelabsolutions/Releasify/compare/1.2.0...1.3.0) (2026-02-06)

### Features

* add bump prerelease mode ([04edd2f](https://github.com/Edgelabsolutions/Releasify/commit/04edd2f551a0504c3ed25e2247457583623b2251))

# [1.2.0](https://github.com/Edgelabsolutions/Releasify/compare/1.1.0...1.2.0) (2026-01-26)

### Features

* add Author info in slack message; upd docs ([f12d036](https://github.com/Edgelabsolutions/Releasify/commit/f12d0360e3ae16d7f5654d025420a6ec97919403))

# [1.1.0](https://github.com/Edgelabsolutions/Releasify/compare/1.0.1...1.1.0) (2026-01-21)

### Features

* add project/repo name in slack message ([525170d](https://github.com/Edgelabsolutions/Releasify/commit/525170dfafd6d0d5847f0fa3321a466ff2bcedd1))

# [1.0.1](https://github.com/Edgelabsolutions/Releasify/compare/1.0.0...1.0.1) (2026-01-20)

### Bug Fixes

* issue with hotfix generation for CHANGELOG.md ([f0b45a2](https://github.com/Edgelabsolutions/Releasify/commit/f0b45a26260b0a00ca466f7e819de508d8ba7465))

## [1.0.0] - 2026-01-18

### Features

- **core:** Initial release of Python-based release automation
- **parser:** Conventional commit parsing with custom type support
- **versioning:** Semantic versioning with prerelease support
- **changelog:** Automatic CHANGELOG.md generation
- **gitlab:** GitLab API integration for releases
- **github:** GitHub API integration for releases
- **config:** Flexible YAML-based configuration
- **docker:** Lightweight Docker image with Python 3.14 Alpine

### Highlights

- Simple single-file YAML configuration
- Custom prerelease tags per branch
- No Node.js dependency
- Transparent, maintainable Python code
- Full control over versioning logic
- Smaller Docker image size
