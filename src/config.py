...
class BranchConfig:
    ...
    prerelease: Optional[str] = None  # Prerelease identifier (e.g., 'dev', 'alpha', 'beta')
...
class Config:
    ...
    DEFAULT_CONFIG = {
        ...
        'commit_types': {
            'breaking': {'bump': 'major', 'keywords': ['BREAKING CHANGE', 'BREAKING-CHANGE']},
    ...
    def __init__(self, config_path: Optional[str] = None, env_overrides: bool = True):
        ...
        self.branches = [
            BranchConfig(**branch) for branch in self.config['branches']
        ]
        self.commit_types = {
            name: CommitTypeConfig(name=name, bump=cfg['bump'], aliases=cfg.get('aliases', []))
    ...
    def _apply_env_overrides(self):
        ...
        if slack_enabled := os.getenv('SLACK_ENABLED'):
            self.config['slack']['enabled'] = slack_enabled.lower() in ('true', '1', 'yes')
